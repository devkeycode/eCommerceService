...
/**
 * Establishing the relation between Product and Cart
 * Since a Product can belong to many carts and cart can have many Products
 * means Products and carts have many to many relationship
 * in many to many relationship, we have to create an association,bridge table
 * cart_products table will be auto generated by sequelize,even we didn't define model for it since its a association/bridge table and we just define the relation among them using the below code
 *
 */
//making many to many relation among cart and product

db.product.belongsToMany(db.cart, {
  through: "cart_products", //table name -association table
  foreignKey: "product_id", //since product table belongs to cart table in 1:many so,fkey will be here product_id here
  otherKey: "cart_id", //other key will be cart_id key from cart_table
});

db.cart.belongsToMany(db.product, {
  through: "cart_products", //table name-association table
  foreignKey: "cart_id", //since cart table belongs to product table in 1:many, so fKey will be cart_id here
  otherKey: "product_id", //other key will be product_id from product_table here
});

/**
 * Establishing the relationship between Cart and User
 * A user can have many cart and a cart belongs to a user only
 */
